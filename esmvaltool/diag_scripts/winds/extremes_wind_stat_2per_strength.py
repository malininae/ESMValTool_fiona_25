...
def obtain_era_data(obs_group, cfg):

    abs_era_cube = iris.load_cube(obs_group[0]['filename'])
    ano_era_cb = eprep.anomalies(abs_era_cube, period='full',
                                 reference={'start_year':cfg['reference_period'][0],
                                            'start_month': 1, 'start_day':1,
                                            'end_year':cfg['reference_period'][1],
                                            'end_month': 12, 'end_day':31,},)
    short_ano_era = eprep.extract_time(ano_era_cb, cfg['reference_period'][0],
                                        1, 1, cfg['reference_period'][1],
                                        12,31)
    ano_era_std = eprep.climate_statistics(short_ano_era, operator="std_dev",
                                           period='full')
    ano_era_cb = ano_era_cb /ano_era_std

    return abs_era_cube, ano_era_cb


def obtain_obs_info(ano_obs_cb, cfg):
    ...
    for i in range(1000): 
        ...
        temp_st_gev = cex.fit_gev(ano_obs_arr[for_fit_indices], 
                                  initial={'location':float(np.around(obs_stat['mle'][0],2)),
                                  'scale':float(np.around(obs_stat['mle'][1],2)), 
                                  'shape':float(np.around(obs_stat['mle'][2],2))}, 
                                  returnValue=ana_year_value, getParams=True)
        try:
    ...
    era_csv_writer.writerow([str(cfg['analysis_year'])+' ERA5 '+cfg['ax_var_label']+' value '+str(ana_year_value)])
    ...
    obs_gev_data={'gev_param_names' : obs_stat['mle_names'],
                  'gev_param_values': obs_stat['mle'],
...
def bootstrap_gev(data_cblst): 

    iter_pool = 1000
    n_years = data_cblst[0].shape[0]
    n_real = len(data_cblst) 
    pool_size = n_real * n_years

    pool_data = list()

    for cb in data_cblst: 
...
def make_hist_figure(data_dic, cfg, uncert_band, border):
    ...
    for model in models: 
        ...
        for exp_key in exp_list:
            ...
            n_bins = np.arange(border[0], border[1]+0.1, 0.2)
            ...
            event_sf = gev.sf(event, -1*w_distr_shape, loc=w_distr_loc, scale=w_distr_scale)
            event_5p = np.nanpercentile(uncert_band[exp_key][model]['return_periods_all'][event_idx,:],5)
            event_95p = np.nanpercentile(uncert_band[exp_key][model]['return_periods_all'][event_idx,:],95)
            try:
                risk_model_row.extend([event_sf, 1/event_sf, event_5p, event_95p, intens, intens_5, intens_95])
            except:
                risk_model_row.extend([np.nan, np.nan, event_5p, event_95p, intens, intens_5, intens_95])
        ...
        ax_hist.set_xlim(border[0], border[1])
        ax_qq.set_xlim(border[0], border[1])
        ax_qq.set_ylim(border[0], border[1])
        ax_surv.set_ylim(1,1000)
        ax_surv.set_xlim(border[0], border[1])
        ax_surv.set_yscale('log')
        ax_surv.grid(color='silver', axis='both', alpha=0.5)
        ax_hist.set_title('(a) Probability density function', fontweight="bold")
        ax_qq.set_title('(b) Quality assessment', fontweight="bold")
        ax_qq.set_ylabel('Data quantile')
        ax_qq.set_xlabel('GEV quantile')
        ax_surv.set_title('(c) Return period', fontweight="bold")
        ax_surv.set_ylabel('years')
        ax_surv.set_xlabel(f"Standardized {cfg['ax_var_label']} anomaly")
        ax_hist.set_xlabel(f"Standardized {cfg['ax_var_label']} anomaly")
        ...
        if model == 'Multi-Model-Mean':
            ...
            + '-' + str(cfg['reference_period'][1])+ ' calculated from '+str(len(models) - 1) +' CMIP6 models', fontweight="bold", fontsize = 'x-large')
        else:
            fig.suptitle(cfg['title_var_label']+' anomalies in '+cfg['region']+' relative to '+ str(cfg['reference_period'][0]) \
        ...
        all_to_nat = list()
        for i in range(len(uncert_band['wind_now'][model]['return_periods_all'][event_idx,:])):
                # we do RPs, so they are flipped
                all_to_nat.append(uncert_band['wind_nat'][model]['return_periods_all'][event_idx,i]/uncert_band['wind_now'][model]['return_periods_all'][event_idx,i])
...
def make_era_dist_figure(obs_info_dic, cfg, border):
    ...
    era_param = cex.fit_gev(abs_cube.data, returnValue=float(ana_year_value), getParams=True)
    era_RP = np.exp(era_param['logReturnPeriod'][0])
    era_loc = era_param['mle'][0] 
    era_scale = era_param['mle'][1] ; era_shape = era_param['mle'][2] 
    ...
    ax_era_hist.set_title('(a) Probability density function')
...
def main(cfg):
    ...
    abs_era_cube, ano_era_cb = obtain_era_data(obs, cfg)

    obs_info = obtain_obs_info(ano_era_cb, cfg)
    ...
    for group in wind_groups_l:
        ...
        for dataset in datasets.keys():
            ...
            for filepath in filepaths:
                ...
                dev_cb = iris.load_cube(select_metadata(dev, dataset=dataset, ensemble=ens)[0]['filename'])
                dev_cb = dev_cb - anom_cb
                dev_cb = eprep.climate_statistics(dev_cb, operator="std_dev",
                                           period='full')
                mod_cb = (mod_cb - anom_cb)/dev_cb.core_data()
        ...
        plotting_dic[group]['Multi-Model-Mean']['uncert'] = bootstrap_gev(ens_cubelist) 
    ...
    border = [np.floor(min_var*1.1), np.ceil(max_var*1.1)]
